...
func (c *Client) MgmtTokenRefresher(mgmtBaseURL string) TokenRefresher {
	...
	return newMgmtRefresher(mgmtBaseURL, c.minter, nil)
}

// newMgmtRefresher builds the refresher against base (scheme://host)
// with the given minter; hc defaults to a 30s-timeout client.
func newMgmtRefresher(base string, m identityMinter, hc *http.Client) TokenRefresher {
	if hc == nil {
