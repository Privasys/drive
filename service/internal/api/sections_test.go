...
func TestDocTreeAndReadSection(t *testing.T) {
	...
	if len(tree.Sections) != 3 || tree.Sections[1].ParentID == nil || *tree.Sections[1].ParentID != tree.Sections[0].ID {
		t.Fatalf("tree unexpected: %s", b)
	}

	// Read the second-half section; its text must be the exact slice.
	code, b = doReq(t, bearerReq(t, "GET",
		fmt.Sprintf("%s/v1/tenants/%s/files/%s/sections/%d", ts.URL, tenantID, fileID, ids[2]), owner, ""))
	if code != 200 {
	...
	if code, _ := doReq(t, bearerReq(t, "GET",
		fmt.Sprintf("%s/v1/tenants/%s/files/%s/sections/%d", ts.URL, tenantID, folder.ID, ids[1]), owner, "")); code != http.StatusNotFound {
